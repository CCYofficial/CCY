...
#if defined(NDEBUG)
#error "Cryptocurrency cannot be compiled without assertions."
#endif
...
if (nHeight == 0) {
             nSubsidy = 1000000 * COIN;
                ...
                nSubsidy = 10 * COIN;
      } else if (nHeight > 390000 && nHeight <= 390010) {
                nSubsidy = 3600000 * COIN;
      } else if (nHeight > 390010 && nHeight <= 400000) {
                nSubsidy = 10 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 5000) {
        ...
        ret = blockValue * 0.000021;

     } else if (nHeight > 390000 && nHeight <= 390010) {
        ret = blockValue * 0.0000025;
...
{
    RenameThread("cryptocurrency-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->nHeight+1);
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.IsProofOfStake() && (pindexPrev->nHeight > 234600) && !IsInitialBlockDownload()) {
