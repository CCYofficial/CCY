...
#include "config/cryptocurrency-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("cryptocurrency-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cryptocurrency.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cryptocurrencyd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 5535, 39795));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CCY/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CCY/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, cryptocurrency, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CCY/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Cryptocurrency specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:5535"));
    ...
    strUsage += HelpMessageOpt("-anonymizecryptocurrencyamount=<n>", strprintf(_("Keep N CCY anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 5536, 39799));
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Cryptocurrency Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_(" ")) + "\n" +
           "\n" +
...
{
    RenameThread("cryptocurrency-loadblk");
...
/** Sanity checks
 *  Ensure that Cryptocurrency is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Cryptocurrency Core is shutting down."));
...
#endif
    // Make sure only a single Cryptocurrency process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Cryptocurrency Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Cryptocurrency version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Cryptocurrency Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Cryptocurrency Core to complete") << "\n";
    ...
    nAnonymizeCryptocurrencyAmount = GetArg("-anonymizecryptocurrencyamount", 0);
    if (nAnonymizeCryptocurrencyAmount > 999999) nAnonymizeCryptocurrencyAmount = 999999;
    if (nAnonymizeCryptocurrencyAmount < 2) nAnonymizeCryptocurrencyAmount = 2;
    ...
    LogPrintf("Anonymize Cryptocurrency Amount %d\n", nAnonymizeCryptocurrencyAmount);
    ...
    /* Denominations
       ...
       For example:
       1CCY+1000 == (.1CCY+100)*10
       10CCY+10000 == (1CCY+1000)*10
    */
