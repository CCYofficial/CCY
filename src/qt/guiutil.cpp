...
#define URI_SCHEME "cryptocurrency"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Cryptocurrency address (e.g. %1)").arg("CH9jJ5A29qf8wUo1a1PWB7RXKnTwY3TBAe"));
#endif
...
{
    // return if URI is not valid or is no Cryptocurrency: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::CCY, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    ...
    //
    //    Cannot handle this later, because cryptocurrency:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CCY, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open cryptocurrency.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Cryptocurrency.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Cryptocurrency.lnk
...
{
    return GetAutostartDir() / "cryptocurrency.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a cryptocurrency.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Cryptocurrency\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the cryptocurrency app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add cryptocurrency app to startup item list
